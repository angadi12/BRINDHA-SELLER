...
export const sendForlive = createAsyncThunk("product/live", async (data, thunkAPI) => {
  try {
    const res = await liveproduct(data);
    return res;
  } catch (err) {
    return thunkAPI.rejectWithValue(err);
  }
...
const productSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(sendForlive.pending, (state) => {
        state.loadinglive = true;
        state.verificationSuccess = false;
        state.errorlive = null;
      })
      .addCase(sendForlive.fulfilled, (state) => {
        state.loadinglive = false;
        state.verificationSuccess = true;
      })
      .addCase(sendForlive.rejected, (state, action) => {
        state.loadinglive = false;
        state.errorlive = action.payload;
      })
