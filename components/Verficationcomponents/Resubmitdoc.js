...
import {
  getVerificationDataThunk,
  getVerificationStatusThunk,
} from "@/lib/Redux/Slices/vendorSlice";
import { useDispatch, useSelector } from "react-redux";
import { useRouter } from "next/navigation";

export default function Resubmitdoc() {
  const { verificationData, Dataloading } = useSelector(
    (state) => state.vendor
  );
  ...
  useEffect(() => {
    dispatch(getVerificationDataThunk());
  }, [dispatch]);
  ...
  useEffect(() => {
    if (verificationData) {
      setFormData({
        BussinessName: verificationData.BussinessName,
        BussinessEmail: verificationData.BussinessEmail,
        BussinessNumber: verificationData?.BussinessNumber.toString(),
        BussinessWebsite: verificationData.BussinessWebsite,
        Bussinesstype: verificationData.Bussinesstype,
        ...
        Bankdetails: {
          ...
          Ifsc: verificationData.Bankdetails.Ifsc,
        },
        Address: {
          State: verificationData.Address.State,
          City: verificationData.Address.City,
          Country: verificationData.Address.Country,
          Place: verificationData.Address.Place,
          Pincode: verificationData.Address.Pincode,
        },
        // Documents: {
        //   AddressProof: verificationData.Documents.AddressProof,
        //   AadharCard: verificationData.Documents.AadharCard,
        //   Pincard: verificationData.Documents.Pincard,
        //   BankPassbook: verificationData.Documents.BankPassbook,
        // },
        Documents: {
          AddressProof: "",
          AadharCard: "",
        ...
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-hidden">
          <ScrollArea className="w-full max-h-[90vh] pb-10 px-4">
            <DialogHeader className="mb-4">
              ...
              <p className="text-red-500 text-center">Remarks-{verificationData?.rejectremark?.map((mark,i)=>(
                <span className="text-sm" key={i}>{mark}</span>
              ))}</p>
            </DialogHeader>
